...
# One class since BN-221. There were three (for index pipelines, universes and
# feature imports), each a copy of the others and each docstring saying so, and
# constraint sets had started borrowing the pipeline one under a name that did
# not fit them.
class FindingsError(InvalidRuleError):
    ...
    at each bad rule, member or row. Index pipelines, universes, feature
    imports and constraint sets all refuse through this one class.
...
# Added in BN-199. Until then a failed job carried `str(exc)` and nothing else,
# so a deliberate refusal and a crash reached a client identically: the one
# place the distinction BN-194 and BN-196 established did not reach. Publishing
# a crash as BEACON_ERROR would be BN-194's original conflation at one remove.
def failure_envelope(exc: BaseException,
    ...
    wrong. A deliberate refusal and a crash therefore reach the client with
    different codes, exactly as they would over HTTP.
    ...
    catch-all instead would put a crash under the code an unregistered
    *refusal* gets, so a client could not tell the two apart.
