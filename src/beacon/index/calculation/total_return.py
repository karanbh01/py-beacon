...
On an ex-dividend date a constituent's price drops, the aggregate drops with
it, and a price index (`PRICE`, the default return type) falls by the whole
distribution. That is what a price index is *supposed* to do, and it is why a
price index understates what an investor actually earned. `TOTAL_RETURN` and
`NET_TOTAL_RETURN` indices reinvest cash distributions from the data source's
...
the index, and it leaves the units untouched. Distributions paid in another
currency are converted into the index currency first; a missing FX pair
refuses.
...
`kind == "cash"`, using the classification the data layer publishes rather
than a list of type strings kept here. A name paying two cash distributions on
one ex-date receives both.

The price path must already contain the drop for this to be right. It does
for a real feed, which quotes prices unadjusted, and for Beacon's synthetic
data, whose generator applies the ex-date drop when it builds the close.
Adding the cash back to an aggregate that never fell would double-count the
distribution.
...
"""
# The action history is BN-98, each action's `kind` is BN-118, and the
# generator's ex-date drop is BN-114. Before this module `IndexCalculator`
# computed a price index and nothing else.
import logging
...
class TotalReturnMixin:
    ...
    def distribution_received(units: dict[Asset, float],
        ...
        Args:
            ...
            rates: Identifier to FX rate into the index currency, as returned
                by :meth:`distribution_rates`. A missing entry converts at 1.0,
                which is correct for a name that already reports in the index
                currency, and that is the only thing a missing entry can mean:
                a name needing a rate that is not there makes
                :meth:`distribution_rates` refuse rather than leave a gap here
                for the 1.0 to fill.
    ...
    def distribution_rates(self,
        ...
        cannot be combined until one is converted.

        Only names the index holds that actually paid are looked up, so a
        quiet day costs nothing. A name already quoted in the index currency
        gets no entry (it converts at 1.0).

        Args:
            per_share: Cash per share by identifier, for this date.
            units: What the index holds, asset to unit count.
            date: The ex-date.
            index_currency: The currency the index reports in.

        Returns:
            dict: Identifier to FX rate into the index currency.

        Raises:
            CalculationError: If a paying name's pair is unknown. A level
                cannot publish "unknown" for one constituent and stay a level,
                so the calculation refuses rather than reinvest the local
                number as though it were in the index currency.
        """
        # Against a single-currency universe a missing conversion is invisible
        # (every rate is 1.0), which is how it survived until the generator
        # grew regions: a yen dividend was counted as though it were dollars,
        # and a twelve-name index came out with a 37% annual yield. BN-189
        # made an unknown pair refuse; it used to log that the distribution
        # was "treated as already in index currency", so a 500-yen payment was
        # reinvested as 500 dollars. The same decision BN-188 took for market
        # caps.
        #
        # The index currency is a parameter rather than read off
        # `self.definition`: `constructor` imports this module for its return
        # types, so a mixin that reached back for the definition would close
        # an import cycle.
