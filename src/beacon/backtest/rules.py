...
A modifier can skip a scheduled rebalance or adjust its trade list. Ships with
`DriftThresholdModifier` (rebalance only when weights have drifted far enough)
and `ExpressionScreen` (drop names that fail an expression at each rebalance).
"""
import logging
...
class BacktestModifier(ABC):
    ...
    def adjust_trades(self,
        ...
        Args:
            trades: The trades the engine generated for this rebalance,
...
# The point-in-time resolution path is BN-142's.
class ExpressionScreen(BacktestModifier):
    ...
    point-in-time path an index rule uses. Resolving it once at the
    ...
    The exit is struck at the price the portfolio was last marked at, with no
    transaction cost, and a held name with no price is carried with a
    warning. If the rebalance already generated a sell for an excluded name,
    that sell is kept as it is and no further exit is added.
